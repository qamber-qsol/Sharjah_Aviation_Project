  ...
  $.extend(EmojiPicker.prototype, {
    ...
    addPickerIcon: function() {
      ...
      if (this.settings.button) {
        ...
        this.$wrapper = this.$el.wrap("<div class='bg-white  border emojiPickerIconWrap p-2 rounded'></div>").parent();

        this.$icon = $('<div class="btn emojiPickerIcon mx-1 " tooltip="Emoji"><i class="fa fa-face-smile-beam"></i></div>')
          .height(40)
          .width(40)
          .addClass(this.settings.iconColor);
          this.$wrapper.append('<div class="MyButton border-top d-flex">')
          
          $('.MyButton').append('<button type="button" onclick="applyFormatting(&quot;bold&quot;)" class="btn mx-1" style="max-width: 40px; max-height: 40px;" tooltip="Bold"><i class="fa fa-bold"></i></button>');
          $('.MyButton').append('<button type="button" onclick="applyFormatting(&quot;underline&quot;)" class="btn mx-1" style="max-width: 40px; max-height: 40px;" tooltip="Underline"><i class="fa fa-underline"></i></button>');
          $('.MyButton').append(this.$icon);
          //$('.MyButton').append('<button type="button"  id="fileButton" class="btn mx-1" style="max-width: 40px; max-height: 40px;" tooltip="Upload Image/Video"><i class="fa fa-photo-video"></i></button>');
          $('.MyButton').append('<button type="button"  id="ai_side_btn" class="btn mx-1" style="max-width: 40px; max-height: 40px;"><i class="fa fa-robot" tooltip="AI Content"></i></button>');
          $('.MyButton').append('<button type="button"  class="btn mx-1" style="max-width: 40px; max-height: 40px;" id="image_Editor"><i class="fa fa-camera-alt" tooltip="Online Media"></i></button>');
          $('.MyButton').append('<button type="button"  id="urlbtn" class="btn mx-1 shadow-none " style="max-width: 40px; max-height: 40px; " tooltip="Web-Link"><i class="fa fa fa-link"></i></button>');
          $('.MyButton').append('<div class="border-left d-flex"><div id="charaterCount" class="m-0 p-2">2000</div><button type="button" id="deleteAll" onClick="DeleteAllContent()" class="btn shadow-none" style="max-width: 40px; max-height: 40px; " tooltip="Web-Link"><i class="fa fa-trash-alt text-danger" aria-hidden="true"></i></button></div>');
    ...
    createPicker: function() {
      ...
      this.$picker.find('.sections')
        .height(parseInt(this.settings.height) - 95); // 40 is height of the tabs
  ...
  function getPickerHTML() {
    ...
    if (localStorageSupport) {
      ...
      for (var i = recentlyUsedEmojis.length-1; i > -1 ; i--) {
        nodes.push('<em><span class="emoji emoji-' + recentlyUsedEmojis[i] + '"></spasectionn></em>');
      }
